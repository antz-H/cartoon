...
description: Builds daily 即梦/JiMeng image prompts for the fixed-character ink picture-book series 墨团与芽仔, writes `作品/{序号} {标题}.md`, and enforces topic constraints (微小但重要、非鸡汤留白、轻悖论/两拍语感可选). Use when the user runs /daily-jimeng-story-prompt, asks to continue the story arc, generate today's page prompt, 续写绘本一页, or assemble prompts from the arc file and template.
---
...
2. **写今日一拍**：须符合 reference 中 **选题约束**；忌与上一页同动作；单场景、可画；可输出 `主题` / `深意` / `本页情节`（**本页情节**直接用于拼装）。
